...
class QuadMeasurer(Configurable):
    ...
    def measure(self, batch, output, is_output_polygon=False, box_thresh=0.6):
        ...
        output: (polygons, ...) shape is [N,k,(x,y)]
        '''
        results = []
        gt_polyons_batch = self.squeeze_list_like_data(batch['polygons'])
        ignore_tags_batch = self.squeeze_list_like_data(batch['ignore_tags'])
        pred_polygons_batch = self.squeeze_list_like_data(output[0])
        pred_scores_batch = self.squeeze_list_like_data(output[1])
    ...
    def squeeze_list_like_data(self,data):
        tem_data = data
        while(type(tem_data) == dict and len(tem_data) == 1):
            tem_data = tem_data[0]
        while(type(tem_data) == Tensor and len(tem_data.size()) > 4):
            tem_data = tem_data[0]
        return tem_data