...
class SegDetectorRepresenter(Configurable):
    ...
    def represent(self, batch, _pred, is_output_polygon=False):
        ...
        # for debug visual
        #self.visualize_threshedMap(segmentation)
        # for debug visual
        boxes_batch = []
        scores_batch = []
    ...
    def boxes_from_bitmap(self, pred, _bitmap, dest_width, dest_height):
        ...
        for index in range(num_contours):
            ...
            box[:, 1] = np.clip(
                np.round(box[:, 1] / height * dest_height), 0, dest_height)
            boxes[index, :, :] = box.astype(np.uint16)
            scores[index] = score
        return boxes, scores

    def unclip(self, box, unclip_ratio=1.2): #unclip 1.5 padding
        poly = Polygon(box)
        distance = poly.area * unclip_ratio / poly.length
        offset = pyclipper.PyclipperOffset()
        offset.AddPath(box, pyclipper.JT_ROUND, pyclipper.ET_CLOSEDPOLYGON)
        tmp_expanded = offset.Execute(distance)
        if len(tmp_expanded) > 1 or len(tmp_expanded) < 1:
           return [1 , 2] #trick for fix debug
        return  np.array(tmp_expanded)
    ...
    #for debug
    def visualize_threshedMap(self,bitmap):
        # plt.imshow(bitmap[0].permute(1, 2, 0).to(device=torch.device('cpu'),dtype=torch.uint8)*255)
        cpu_map = bitmap[0].cpu().to(dtype=torch.uint8)
        np_map = cpu_map.numpy()
        img = Image.fromarray(np_map[0]*200,mode='L')
        img.save("demo_results\\mask\\007.png")
