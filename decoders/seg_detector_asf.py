...
class SegSpatialScaleDetector(nn.Module):
    ...
    def forward(self, features):
        ...
        binary = self.binarize(fuse)
         ...
         # tempoary comments it for debug visualize threshold
        if self.training:
            ...
            return binary
         # tempoary comments it for debug visualize threshold end
        #comment this for train or eval
        #thresh = self.thresh(fuse)
        #thresh_binary = self.step_function(binary,thresh)
    ...
    def step_function(self, x, y):
        if self.__punish_map != None:

            if self.__punish_map.shape == x.shape:

                return torch.reciprocal(1 + torch.exp(-self.k * (x - y) * self.__punish_map))

            else:

                raise Exception('bad punish shape')
            
        return torch.reciprocal(1 + torch.exp(-self.k * (x - y)))

    def set_punish_map(self,punish:torch.Tensor,device:torch.device):
        (b,c,h,w) = punish.shape
        self.__punish_map = torch.zeros(size=(b,c,h,w),dtype=torch.float32,device=device)
        self.__punish_map[punish == 2] = 1
        self.__punish_map[punish == 1] = 0.875
        self.__punish_map[punish == 3] = 1.25
        # for bi in range(b):
        #     for ci in range(c):
        #         for hi in range(h):
        #             for wi in range(w):
