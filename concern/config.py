...
class Config(object):
    ...
    def compile(self, conf, return_packages=False):
        ...
        for path in conf.get('import', []): #get import yaml
            parent_conf = None
            if type(path) == list:
                parent_conf = self.load(os.path.join(*path))
            else:parent_conf = self.load(path)
    ...
    def load(self, state_name, **kwargs):
        ...
        if state_name in kwargs:
            setattr(self, state_name, self.create_member_from_config(
                (kwargs[state_name], cmd))) #这里kwargs[state_name] 为子kwargs ，在不同子类中
        else:
