...
class Trainer:
    ...
    def train(self):
        ...
        while True:
            ...
            for batch in train_data_loader:
                self.update_learning_rate(optimizer, epoch, self.steps) # batch is OrderedDict['image', 'gt', 'mask', 'thresh_map', 'thresh_mask','thresh_punish_mask']
                batch['thresh_map'] = batch['thresh_map'] * 0.5
    ...
    #only for debug
    # def save_float_tensor_to_image(self,tensor_img:torch.Tensor,file_str:str):
    #     scaled_tensor = ( tensor_img * 255).clamp(min=0,max=255).to(torch.uint8)
    #     pil_image = None
    #     if len(tensor_img.shape) == 4:pil_image = transforms.ToPILImage()(scaled_tensor[0])
    #     else:pil_image = transforms.ToPILImage()(scaled_tensor)
    #     pil_image.save(file_str)
